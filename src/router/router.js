...
function Main() {
  const { routes } = JSON.parse(window.sessionStorage.getItem('menuList'));
  return (
      <main className="ccp-app-main">
          <Switch>
              {routes.map(v => {
                  return <Route key={v.url} path={v.url} component={componentMap[v.code]} />;
              })}
              
              {routes.length > 0 ? <Route to="/404" /> : <Route to="/404" />}
          </Switch>
      </main>
  );
}
...
const getRouter = () => (
  <HashRouter history={hashHistory}>
    <Switch>
      <Route path="/IndexBody" component={RouterMap.IndexBody} data='aaa'/>
